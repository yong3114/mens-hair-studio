...
export async function getDashboard() {
  const [leads, customers, appts, systems, consumables, payments] = await Promise.all([
    supabase.from('leads').select('id,stage,follow_up_date'),
    supabase.from('customers').select('id'),
    supabase.from('appointments').select('id,scheduled_at,status,service_type,customers(name),profiles!appointments_assigned_user_id_fkey(full_name)').gte('scheduled_at', new Date(new Date().setHours(0,0,0,0)).toISOString()).order('scheduled_at').limit(30),
  ...
  ;[leads,customers,appts,systems,consumables,payments].forEach(ok)
  return { leads:leads.data, customers:customers.data, appts:appts.data, systems:systems.data, consumables:consumables.data, payments:payments.data }
}
...
export async function getCustomer(id){
  const [c,a,s,h,p,cr,m] = await Promise.all([
    ...
    supabase.from('services').select('*,profiles!services_technician_id_fkey(full_name)').eq('customer_id',id).order('completed_at',{ascending:false}),
...
export async function listAppointments(start,end){
  let q=supabase.from('appointments').select('*,customers(id,name,phone),profiles!appointments_assigned_user_id_fkey(id,full_name)').order('scheduled_at')
...
export async function listHairSystems(){ return ok(await supabase.from('hair_systems').select('*,customers(name)').order('created_at',{ascending:false})) }
...
export async function createService(payload,consumptions=[]){
...
export async function listPayments(){ return ok(await supabase.from('payments').select('*,customers(name)').order('created_at',{ascending:false}).limit(300)) }
...
export async function exportAll(){
  ...
  const out={exported_at:new Date().toISOString(),version:'2.0.0',data:{}}

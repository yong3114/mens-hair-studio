-- MEN'S HAIR STUDIO V2.1 — PREP SCHEMA
...
create table if not exists public.leads (
 ...
 name text, whatsapp_name text, phone text, area text, source text not null default 'Other',
...
create table if not exists public.customers (
 id uuid primary key default gen_random_uuid(),
 name text, whatsapp_name text, phone text, email text, area text, status text not null default 'active' check(status in ('active','inactive')),
...
create table if not exists public.appointments (
 ...
 status text not null default 'confirmed' check(status in ('tentative','confirmed','in_progress','completed','cancelled','no-show')),
...
create table if not exists public.services (
 ...
 started_at timestamptz, completed_at timestamptz, status text not null default 'completed' check(status in ('in_progress','completed','cancelled')), next_maintenance_date date,
